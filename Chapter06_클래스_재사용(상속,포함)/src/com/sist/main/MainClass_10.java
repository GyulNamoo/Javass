 ...
 *  --------  -------
 *            상속에서 접근 지정어
 *   오버로딩 / 오버라이딩 => 편집(97%) , 7장(추상클래스 / 인터페이스 ) =>85%
 *              | 라이브러리                           | Spring기반
 ...
 *                          => 다른 클래스와 연결
 *                 private : 멤버변수 => 데이터 노출 방지(데이터 손상 보호)
 *                           => 다른 클래스에서 사용
 *                              => 변수 기능의 메소드 제작
 *                                 ------------------
 *                                 메모리 저장 / 메모리에 읽어오기
 ...
 *               => 인스턴스메소드, 인스턴스변수는 사용할 수 없다
 *               => this가 없다
 *            인스턴스메소드 : 인스턴스메소드 , 인스턴스 변수 , static변수 , static 메소드 사용이 가능      
 *            ------------------------- this를 가지고 있다          
 *                               
 *                                  */
...
{
	public void display1(BCD this) {} //멤버메소드
	//public static void display2() {} // 공통메소드
	//public final void display3() {} // 종단메소드
}
// 인스턴스 메소드 , 인스턴스 변수 => 중첩, static은 데이터베이스 => MyBatis => 2차 (Spring)
// => 실무 : SpringFramework + MyBatis => MySQL 
class DGB
{
	//this. ===> DGB가 가지고 있는 변수
	// 생성자 , 멤버 메소드에만 존재 => static 메소드는 this가 없다 => 멤버 변수를 사용 할 수 없다
	private String name;
	private boolean login;
