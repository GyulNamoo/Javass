 ...
 *     => 구현이 불가능
 *        ---
 *         제어문 / 연산처리 / 파일 읽기 / 크롤링 / 메소드 호출 ... => 메소드처리 , 생성자, 초기화블록을 이용
 *     => 선언만 가능하다
 *        -----------
 *        1. 변수 : 한개만 저장이 가능
 *        2. 클래스 (사용자 정의 데이터형)
 ...
 *      protected : 같은 패키지에서만 접근이 가능 + 상속을 받은 경우에는 다른 패키지 접근이 가능
 ...
 *       호출 => 메소드에 있는 모든 문장 수행 ===> 호출된 위치 복귀
 *       -----------------------------------------------
 *       * 메소드는 호출시마다 => 메소드 처음부터 다시 수핼
 *        
 *   ----------------------------------------------------------------------------
 *   => 패키지 / 임포트
 *   => 데이터 보호 : 캡슐화
...
public class 클래스구성요소_변수 {
	 ...
	 *         aa.a
	 *      }
	 *      public void ccc() => 객체 자시닝 가지고 있는 메소드
	 *      {
	 *      	=> aaa() / a / b
	 *      }
