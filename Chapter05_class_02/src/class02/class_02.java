 ...
 *         => 명사적 => 변수 
 *         => 동사적 => 메소드 : 점심을 먹는다 , 시험본다, 게임을 한다...(동적)
 *         
 ...
 *             = 한개의 파일(.java)에 여러개의 클래스 를 만들 수 있다
 *             = 클래스 형태
 *               = 데이터형 클래스
 *                  => 변수만 설정 => 사용자 정의 데이터형
 *                                 ~VO(String), ~DTO(MyBatis), ~Bean(JSP)
 *               = 액션 클래스                  
 *                  => 메소드 설정 => 기능만 처리 
 ...
 *               커마일시 저장 => 한개의 메모리 생성 => 정적변수 => static 
 *            *** 초기화
 *                인스턴스 변수 : {} , 생성자
 *                static 변수 : static{} , 생성자 => 자동 저장이 가능 => 생성자를 ㅣㅇ용하지 않는다 
 *            *** 변수활용 => 메소드
 *         }	
 */
