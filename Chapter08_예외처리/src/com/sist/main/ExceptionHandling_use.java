...
public class ExceptionHandling_use extends JFrame implements ActionListener{
	// ExceptionaHandling_use => 윈도우를 확장 => 상속(단일 상속 기본)
	JTextField tf;
	JButton b1, b2;
	JLabel la = new JLabel("",JLabel.CENTER);
	// 클래스 => 변수 (객체) => 변수 사용 (기본형, 배열, 클래스) => 멤버변수로 사용이 가능
	// 초기화, 화면UI, 서버 연결 => 생성자에서 주로 처리 
	// 전체 변수
	int com; // 난수
	int user; // 사용자 입력값
	// 메소드 2개이상에서 사용 => 멤버변수 => 1. 입력값을 받는 경우, 2. 비교시에 사용
	// 메소드 1개에서만 사용 => 지역변수 
	// 처리는 입력 => 엔터시 => 입력값을 받는다 , 비교 => 결과값 출력
	
	public ExceptionHandling_use() 
	{
		tf = new JTextField();
		b1 = new JButton("시작");
		b2 = new JButton("종료");
		...
		tf.setEnabled(false); // false => 비활성화
		// 윈도우 추가
		add(tf);
		...
		// 버튼 클릭 => 등록 => 메소드 자동 호출
		b1.addActionListener(this); // actionListener => 어디 있는지...!!
	...
	public static void main(String[] args) {
		...
		// 버튼이나 JTextField처리(엔터) => 고정 => 구현을 할 수 없어서 선언만 해서 넘겨준다 => 프로그램에 맞게
		@Override
		public void actionPerformed(ActionEvent e) {
			...
			else if(e.getSource() == tf) // 입력후에 엔터라면
			{
				...
				} catch (Exception ex)
				{
					...
					tf.requestFocus();
					// 다시 처음부터 동작을 하게 만든다
					return; // 메소드에서 return을 사용하면 => 메소드가 종료 
				}
