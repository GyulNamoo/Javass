 ...
 *             -------------- if(조건문 처리) => 유효성 검사
 *          2. 프로그래머의 실수 : 배열의 범위 추가, 반복수행 ... 클래스 메모리 할당이 없는 상태에서 사용
 *                           변수의 초기화가 없는 상태 ..., 잘못된 입력
 *       => 예외처리
 *             = 사전에 에러 방지 
 *             = 비정상 종료를 방지하고 => 정상 상태 유지
 *             = 예외처리 => 에러 발생시 에러부분을 건너뛴다
 *               ------------------------------- 에러를 수정해 주는 것은 아니다 
 *       => 예외처리 방법
 *          ------------------------------------
 *          = 예외 복구 : try ~ catch
 *                     ---   -----
 *                     정상수행  에러발생시 처리 => 복구 
 *          = 예외 회피 : 처리하지 않고 => 예외 발생시 시스템 에 의해 처리
 *                     메소드뒤에 첨부 
 *                     메소드() throws 예외처리
 *          ------------------------------------
 ...
 *        NullPointerExcetion => 메모리 할당이 없는 상태에서 객체 사용시 ... 
 *        ------------------- String , 윈도우 관련
 ...
 *           ------------------------------------
 *           |                                  | 선택
 *         IOExcetion                        RuntimeExcetion : 실행시 에러 => 필요시에는 예외처리 한다
 *         SQLException                            |
 ...
 *             상속도 => 상속을 내리는 클래스는 하위 클래스와 예외를 포함하고 있다
 *             => 한번에 처리가 가능하다
 *             
 *             예) 
 *                -----------------------------------------------------------
 *                배열을 선언 => ArrayIndesOutOfBoundsExveption
 *                정수를 입력을 받아서 => 정수가 입력이 안된경우 => numberFormatExcetion)
 *                나누기 한다 => ArithmetiocException
 *                -----------------------------------------------------------
 *                => 나눠서 잡는다
 *                => 통합해서 잡는다 
 *                
 ...
 *               2. 에러가 있는 경우 => 해당 에러를 등록학 catch를 찾는다
 *            } catch (예외처리클래스)
 *            {
 *               에러 발생시에 복구하는 위치
 *            }
 ...
 *               문장 1
 *               문장 2
 *               문장 3   => 에러발생 => catch로 이동   ---- => 예외처리를 하지 않는 경우에는 종료 
 *               문장 4
 *               문장 5                             ---- 건너뛴다
 *            } catch(예외처리) 
 *            {
 ...
 *            => 1,2,6,7,8 => 종료
 *            
 *            try절 : 정상수행이 가능한 소스코딩을 하는 부분 => 모든 프로그램 소스가 서술
 *            => 1개만 설정
