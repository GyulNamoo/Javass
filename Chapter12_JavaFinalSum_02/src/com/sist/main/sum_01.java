 ...
 *           ---------------------------> 확장
 *           => 오버라이딩 => 접근지정어의 확장 은 가능 / 축소는 불가능 
 *              ----------------------------------------
 *       => static 메소드 / static블록에서는 인스턴스를 사용할 수 없다 
 *            => 인스턴스를 사용하기 위해서는 반드시 메모리 할당시 사용
 *               --------------------------------------- 매개변수 
 *       => 인스턴스메소드 / 인스턴스 블록 => 인스턴스변수, static변수 사용이 가능
 *               
 *       예)
 *           class A
 *           {
 ...
 *                    추상 클래스            인터페이스   =====> 75%
 *    ==================================================
 *     상속               단일상속             다중상속   =========> 추상 클래스의 단점을 보완
 *    ==================================================
 *     사용방법
 *                   extends 추상클래스명 implements 인터페이스
 *                   -------          ----------
 *    ==================================================
 *     변수           멤버변수                  상수
 *    ==================================================
 ...
 *                                    => 서버 IP / 파일명이 다르다 / 배열 범위 초과 / 0으로 나눈다 
 *                                       |
 *                             -------------------------
 *                             |                       |
 ...
 *                                 0번부터 시작
 *                     ArrayList : 데이터베이스 => 추출한 데이터를 브라우저로 전송, 윈도우로 전송 
 *                     ---------
 *                     add() : 추가
 *                     size() : 저장 갯수
 *                     get() : 데이터 읽기 
 ...
 *                     HashMap : 클래스를 모아서 관리, 웹에 전송, 웹에서 응답, 쿠키에 저장 , 세션에 저장 
 *                     -------
 *                       put(키,값) : 추가
 *                       size() : 저장 갯수
 *                       get() : 데이터 읽기
 ...
 *                       isEmpty()
 *                      
 *             **** 저장된 데이터형 => 프로그래머가 어떤 데이터를 저장ㅎ라 지 모른다
 *                                --------------------------------
 *                                 => 어떤 데이터든 상관없이 받을 수 있게 제작 => Object
 *                                 => 리턴형 / 매개변수 => Object로 되어 있다
 *                                                    ------
 *                                                       | 가장 큰 데이터형이다 => 형변환을 해서 사용
 *            **** Object를 전체 변경이 가능하게 만든다 (프로그램에 맞는 데이터형 변경)
 *                 => 데이터형을 통일화
 *                 => 가독성 => 데이터 확인이 가능
 *                 => 형변환이 없기때문에 간결
 *                 => 제네릭 
