 ...
 *               5) 공백을 사용 할 수 없다
 *               -------------------------- 식별자
 *               ** 변수는 중복 사용이 불가능 ( 한개의 메소드에서 같은 변수 사ㅏ용이 안된다)
 *               ** 메소드 같은 이름의 사용이 가능
 *               ** 약속 사항(자바 개발자) => 소문자로 시작한다
 *                  => 단어가 2개 일 때
 *                     fileName() => 두번째 시작하는 단어대문자로 시작(헝거리식 표기법)
 *                                                    _
 *               ** 결과값 / 요청값을 받는다
 *                  ----    ----
 *                  리턴형   매개변수
 ...
 *                             
 *             ** 클래스
 *                = 사용자가 데이터형 => 데이터만 저장할 목적 => ~VO,~DTO,~Bean
...
public class method_01 {
	...
	//      공유   결과값이 없다  메소드명  매개변수
		...
		// void java.io.PrintStream,println(String X) => 자체처리
