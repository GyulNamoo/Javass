 ...
 *       1. 묶음 
 *          변수 여러개를 묶어서 관리 
 *          변수 여러개 => 제어문 사용이 어렵다 ==> 배열( 이름이1개 => 구분자는 인덱스)
 *          -------- 평문을 사용한다                                   |                  |
 *                                                             0번부터 순차
 *          ***배열은 변수가 3개 이상일 경우
 *              *** 3개 => 사용 용도가 같을 경우 ...
 ...
 *                 2) 연산처리시에는 큰데이터형으로 변경 => 연산처리(같은 데이형이어야 한다)
 *                    10 + 10.5 
 *                    --   ----
 *                    10.0(double) => 연산처리
 *                    'A' + 1
 ...
 *                        
 *                        && => 범위에 포함
 *                              => 조건 2개가 동시에 true때만 true => 나머지 false
 *                        || => 범위를 벗어나는 경우
 *                              => 조건 2개중 1개 true때만 true
 *              -------------------------- if, for, while
 *       삼항연산자                                 <======= 9
 ...
 *            => 비교연산자 / 논리연산자
 *                         -------- 범위 지정할 경우
 *         ** if을 여러개 사용하면 독립적이기 때문에 모든 조건을 검색 => 속도가 느려질 수 있다
 *         ** 여러 문장을 동시에 수행이 가능
 *           if~else => 선택 조건문
 ...
 *            ---------------------------------------------------------------------------------
 *            
 *            다중 조건문 => 해당 조건문장만 수행 => 키보드 입력 (게임) , 메뉴
 *              if(조건문)
 *              { 
 *                 조건문 true => 수행 => 종료 
 *              }          |false 다음 조건문을 찾는다
 ...
 *                   
 */
public class for_for_06 {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		
		// 1. 컴퓨터가 난수 발생 => 1~9사이의 중복이 없는 숫자 => 3개
		int[] com = new int [3];
		for(int i=0; i<com.length;i++) // 3번 반복
		{
			com[i]=(int)(Math.random()*9)+1; //1~9사이
			//중복여부 확인
			for(int j=0; j<i; j++) //생성된 난수만큼
			{
		...
		// 2. 사용자 입력 요청
		 while(true) //반복횟수가 없다 => 정답시에 종료 => 무한루프
		 {
			 ...
			 if(input<100 || input >999)
			 {
				  ...
				  *    }
				  *    
				  *    break 는 while,for이나 상관없이 반복문을 중단
				  *    
				  */
			 }
			// 세자리 정수를 입력한 경우
			...
			// 정상적으로 입력이 되었다면
			// 힌트
			 int s = 0, b=0; //s는 숫자와 자리수가 맞는 경우, b는 숫자는 있고 자리수가 맞지 않은 경우
		...
		// 3. 비교 => 힌트
		// 4. 정답여부 확인 => 정답이면 종료 / 오답이면 => 다시 입력이 가능
		//                  break        continue
