 ...
 *          FileReader
 *          
 *      1. 메모리에 저장 => 프로그램 종료가 되면 모든 데이터가 사라진다 
 *         --------- 속도가 빠르다
 *           | 영구적으로 저장하는 장치
 *             -----------------
 *              파일
 ...
 *                       => 임시 저장 
 *      2. 파일의 정보 => 376page => 파일 입출력
 *         -------     -------
 *                     |377page에 나오는 메소드
 *          File : 파일, 폴더정보를 가지고 있다 
 *          ---------------------------
 *          File 정보와 관련된 메소드
 *          File 생성자 
 *          ---------
 ...
 *            7) 쓰기 가능 여부 => catWrite() : boolean
 *            8) 수정일 => lastModified() : long
 *            ***9) 파일 정보를 한번에 읽기(폴더) => listFiles() : File[]
 *            10) 파일 여부 확인  => ifFile() : boolean
 *            11) 폴더 여부 확인  => ifDirectory() : boolean
 *          File 제어하는 메소드 
 *            ***1) 폴더 생성 : mkdir()
...
public class Library_01 {
	...
	public static void main(String[] args) {
		...
		System.out.println("***파일 여부 확인:" + file.isFile());
		//응용 
