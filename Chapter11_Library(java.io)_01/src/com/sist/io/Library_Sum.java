 ...
 *      java.io
 *      -------
 *      | 특징 : 데이터이동 통로를 만들어서 데이터를 입출력을 한다 
 *             -----------
 *              | 스트림 : 단점 (단방향이다) => 빨대프로그램 
 ...
 *            송신/수신 => 1byte 전송 => 자바에서는 각국의 언어를 사용할 수 있게 2byte를 사용할 수 있게 제작
 *            ---------------- 
 *            1byte => 바이트 스트림
 ...
 *          입출력
 *           --------------------- 바이트 스트림 --------------------------
 *           => 읽기메소드 : read()  => 한글자 읽기 read, read(byte[])
 *           => 쓰기메소드 : write() => 한글자 출력 write (byte[]) write(int)
 *           ----------------------------------------------------------- 파일 닫기 : close()
 *              FileInputStream / FileOutputStream
 ...
 *                                   
 *         ==> 파일을 읽어서 => ArrayList에 저장후에 데이터를 활용
 *                          --------- 배열 
 *                          
 *          => Properties
 *             ----------- 파일의 확장자 : .properties
