 ...
 *                   System.out.printlna.toString()) => 명시적 
...
{
	private int sabun;
	private String_01 name;
	//사용자로부터 값을 받아서 초기화
	public Sawon(int sabun,String_01 name)
	{
		this.sabun = sabun;
		this.name = name;
	}
...
class Student
{
	...
	public Student(int hakbun, String_01 name)
	{
		this.hakbun = hakbun;
		this.name = name;
	}
	// hashCode변경 => 라이브러리는 필요한 경우에 
	@Override
	public int hashCode() {
		// TODO Auto-generated method stub
 ...
 *           ------------------------
 *            상수 ( 변수는 존재하지 않는다)
 *            (public static final) int a = 100; // 반드시 선언과 동시에 초기값을 설정 => 변경이 불가능 
 *           ------------------------
 *            구현이 안된 메소드
...
class Box implements Cloneable
{
	...
	// throws문장에 뒤에 붙는 예외 클래스는 => RuntimeException관련은 없다 => 예외처리를 하고 사용한다
...
{	//c언어 => Member(), ~Member()
	...
	private String_01 name;
	// 생성시에 호출 => 생성자
	public Member(int mno,String_01 name)
	{
		this.mno = mno;
		this.name = name;
	}
...
class Food
{
	...
	public Food(int fno,String_01 name)
	{
		this.fno = fno;
		this.name = name;
		// 지역변수 우선순위
...
public class Object_01 {
	...
	public static void main(String_01[] args) {
		// TODO Auto-generated method stub
		/*Sawon s1 = new Sawon(1, "홍길동"); // 데이터베이스에 데이터를 읽기 / 파일 데이터 읽기
		System.out.println(s1); // sql.toString() => toSring()
...
//		 *   Box box4 = box1;
		...
		try {
			...
			System.out.println("box2=>" + box2); // 메모리 주소가 다른 경우에느 새로운 메모리가 생성
			//제어를 따로 할 때 사용
		...
		// equals = 재정의 => 오버라이딩을 하기 전까지 => 주소값 비교 
		if(f1.equals(f2))
		{	
