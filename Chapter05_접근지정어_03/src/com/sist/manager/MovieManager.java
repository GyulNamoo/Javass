 ...
 *      클래스 : 메소드를 이용하거나 멤버변수를 사용 => 다른클래스에서 사용 => 메모리 할당 => public
...
public class MovieManager {
	...
	public static Movie[] movies = new Movie[1938]; // 자동으로 메모리에 저장 => new가 필요없다 => 생성자가 필요 없다
	// static은 변경이 없는 경우 => 고정 (모든 사용자 같은 내용을 보고 있는 경우) => 공유 
	// 데이터 여러개일때 => 메모리 공간을 한개만 이용해서 접근 ... (static)
	// 인스턴스 변수는 각자 따로 저장 (new) => 보안 => private을 이용해서 저장 => 은닉화
	// 초기화 
	/*
	 *   
	...
	static // 컴파일러에 의해 자동 처리 => 호출하지 않는다 (생성자는 호출해야 된다) 
	{
		try {
			...
			int i = 0; // read() => char을 읽지 않고 ASC를 읽어온다 => 문자 번호를 읽어 온다
			// A => 65=> i 는 읽어온 정수를 저장할 변수 => char로 변환
	...
	public static Movie[] getMovieData(int page)
	{
		...
		int rowSize = 20;
		int pagecnt=(page*20)-rowSize; // 데이터를 저장할 시작점 
