 ...
 *              -------------------------------------------------------
 *       => StringBuffer : 임시 저장장소를 만들어서 문자열 결합 => 최적화
 *          => append() : 문자열 결합 
 *       => Math : 수학연산
 *          => random() : 임의 추출
 *          => ceil() : 올림
 ...
 *          => gc() : 메모리 회수
 *          => exit() : 프로그램 종료
 *       => ***Wrapper : 기본 데이터형(int,long,double)를 쉽게 사용이 가능하게 기능 부여 
 ...
 *                          ML => <a> : 태그 => 사용자 정의
 *                          HTML / XML은 오라클로 연결하는 언어가 아니다 ... (화면 출력) 
 *                        | 자바는 오라클에 있는 데이터를 관리
 *      -------
 *      화면 출력 (JavaScript) => NodeJS 
 *      String / ArrayList / Integer => 웹 에서 사용하는 3대 클래스      
 */
// 323 page => Random 
// Random => 임의로 추출 => 정수
/*
 *     정수 => nextInt() => 정수형 전체 => 0~21억4천
 *           ----------
 *           nextInt(10) => 0~9
 *           nextInt(46) => 0~45
 *                   --- 제외 
