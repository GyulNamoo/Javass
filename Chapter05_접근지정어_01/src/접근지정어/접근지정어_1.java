 ...
 *             데이터 선언
 *              => 멤버변수
 *                 = 인스턴스변수 : new사용시마다 메모리가 따로 생성 => 여러개 저장이 가능
 ...
 *                    --------------------------------- 유스케이스 다이어그램
 *                    마이페이지
 *           Back-End (자바,JSP,Spring,오라클)
 *           Front-End(JavaScript) => Ajax, next, Vue, React
 *            ---------------
 *             데이터 초기화 => 클래스 영역 안에는 선언만 가능 => 구현(파일읽기, 화면 UI, 웹연결, 연산처리, 제어문)
 *                생성자 === 모든 클래스에서 반드시 한개이상을 포함한다
 *                          => 생략할 경우에는 컴파일러가 자동으로 생성 : default생성자
 *                          => default생성자는 매개변수가 없는 생성자 default
 *                          class A
 *                          {
 ...
 *                  = 여러개를 사용이 가능 => 오버로딩
 *                  = 객체 생성시 반드시 호출
 *                  = 시작과 동시에 처리해야되는 기능이 있는 경우 => 화면 UI
 ...
 *                
 *                데이터를 저장할 메모리를 만들고 초기값이 설정 => 바로 사용이 가능하게 만든다
 *                ------------------------------------------------------------
 *                 자바 번역) 
 *                          new 클래스명() 
 ...
 *          ---------------------------------- 접근 지정어
 *          -----------------------------------------------------------------------------------
 *                        자신의 클래스   같은 패키지   같은 패키지+상속(다른패키지에 접근)   모든 클래스  
 *          ----------------------------------------------------------------------------------- 
 *            **private       O                                                   => 본인
 *                           멤버변수 
 *          -----------------------------------------------------------------------------------
