package method;
/*
 *    메소드 : 기능처리 (사용자 요청처리) => 한개의 기능만 수행이 가능하게 만드는 명령문의 정렬
 *           명령문
 *            1. 변수 선언
 *            2. 연산 처리
 *            3. 제어문처리
 ...
 *       => 3. 재사용 => main 다른 클래스에서 호출이 불가능
 *               | ==> CTRL+C (O)
 *               | ==> 있는 그대로 호출해서 사용 => 메소드
 *       => 4. 객체지향 프로그램은 클래스와 클래스의 상호 연결 
 *       => 클래스 / 기능 / 조립
 *                  |      | => 메인보드 (main이 있는 클래스)
 *                 CPU
 *                 메모리
